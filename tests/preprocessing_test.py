...
def test_complex_query(attributes):
    ...
    true = """SELECT Patient.name.given, Weight.valueQuantity.value, Weight.valueQuantity.unit, MedicationRequest.medicationCodeableConcept.coding.display, MedicationRequest.dosageInstruction.doseAndRate.doseQuantity.value, MedicationRequest.dosageInstruction.doseAndRate.doseQuantity.unit, Patient.birthDate FROM Patient INNER JOIN Observation as Weight ON Weight.subject = Patient.id INNER JOIN MedicationRequest ON MedicationRequest.subject = Patient.id WHERE Weight.code = http://loinc.org%7C29463-7"""
    assert query == true
